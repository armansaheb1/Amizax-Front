<template>
  <div class="container px-0" style="font-family: 'UD'">
    ...
    <h2 v-for="item in trades" v-bind:key="item">{{ item.name }}</h2>
    ...
    <div class="list" style="height:165px; overflow-x:hidden ;overflow-y:scroll ; border: solid lightgrey .2px ; border-radius: 0 0 5px 5px; font-size: 13px!important" id="my-list-id">
      <button v-for="item in trades" v-bind:key="item" class=" curbtn" onMouseOver="this.style.background='rgba(150, 150, 150, 0.4)'" onMouseOut="this.style.background='rgba(0,0,0,0)'"   type="button"  style="height:55px; width: 100% ; background: none ;border-style: none; border-bottom: solid .2px lightgrey ;border-shadow:none margin:0 ; font: 13px 'UD'!important;padding-left:10%"  @click="buttonchange('USDT')"  selected >
        <template style="margin:9px; margin-bottom: 5px">
          <img style="width:32px;height:32px;border-radius: 50%; background-color: grey;" :src="item.get_image"   alt="">
          <img style="width:32px;height:32px;border-radius: 50%; background-color: grey; margin-bottom: 5px; margin-left: 10px; margin-top: -16px" :src="item.get_image"   alt="">
        </template>
        <br>{{item.brand}}</button>
    </div><br><br>
