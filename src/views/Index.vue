<template>
  <div>
    <div class="ban">
      ...
      >
        <b-col style="background: none !important ; margin: 0; padding: 0">
          <div
            class="mainban"
            style="height: 150px; margin: auto; text-align: center"
          >
            <div style="border-radius: 20px">
              ...
              <h2 class="mainsubhead" style="color: black">
    ...
    >
      ...
      <tbody v-if="info && !sorted" style="font: 20px 'UD'; max-height: 320px">
        <tr v-for="(item, name, idx) in info" v-bind:key="idx + 100001">
          <td v-if="name.replace('USDT', '') === 'USDT'" class="cent">
            <template class=""
              ><br class="omobile" /><br class="omobile" /><img
                loading="lazy"
                style="width: 48px"
                src="https://raw.githubusercontent.com/ErikThiart/cryptocurrency-icons/master/64/tether.png"
                alt="" /></template
            ><br />{{ "USDT" }}
          </td>
          <td v-if="name.replace('USDT', '') === 'USDT'" class="cent">
          ...
          >
            ...
            <router-link
              ...
              >خرید </router-link
            ><router-link
              :to="`/cpwallets/${'USDT'}/deposit`"
              class="btnfont btn btn-dark walbtn"
              style="font: 16px 'Yekan'"
              >واریز</router-link
            ><router-link
              :to="`/cpwallets/${'USDT'}/withdraw`"
              class="btnfont btn btn-dark walbtn"
              style="font: 16px 'Yekan'"
              >برداشت</router-link
            >
        ...
        <tr v-for="(item, name, idx) in info" v-bind:key="idx + 10000">
          <td
            v-if="
              (name.replace('USDT', '') === 'BTC') |
                (name.replace('USDT', '') === 'ETH') |
                (name.replace('USDT', '') === 'TRX') |
                (name.replace('USDT', '') === 'DOGE') |
                (name.replace('USDT', '') === 'SHIB') |
                (name.replace('USDT', '') === 'BNB') |
                (name.replace('USDT', '') === 'BCH') |
                (name.replace('USDT', '') === 'XRP')
            "
            class="cent"
          >
            <template class=""
              ><br class="omobile" /><br class="omobile" /><img
                ...
                :src="`/icons/color/${name
                  .replace('USDT', '')
                  .toLowerCase()}.svg`"
                :onerror="`javascript:this.src='/icons/color/${name
                  .replace('USDT', '')
                  .toLowerCase()}.png';`"
                alt="" /></template
            ><br />{{ name.replace("USDT", "") }}
          </td>
          <td
            v-if="
              (name.replace('USDT', '') === 'BTC') |
          ...
          >
            <br /><a
              v-if="parseFloat(item.change).toFixed(1) > 0"
              style="color: green"
            >
              {{
                parseFloat(item.change).toFixed(1).toString().replace("-", "")
              }} </a
            ><a
              v-if="parseFloat(item.change).toFixed(1) < 0"
              style="color: red"
            ...
            <a v-if="!parseFloat(item.change)">-</a>
          </td>
          <th
            style="padding-top: 25px"
            v-if="
          ...
          >
            ...
            <router-link
              :to="'/buy/' + name.replace('USDT', '')"
              class="btnfont btn btn-dark walbtn"
              style="font: 16px 'Yekan'"
              >خرید </router-link
            ><router-link
              :to="`/cpwallets/${name.replace('USDT', '')}/deposit`"
              class="btnfont btn btn-dark walbtn"
              style="font: 16px 'Yekan'"
              >واریز</router-link
            ><router-link
              :to="`/cpwallets/${name.replace('USDT', '')}/withdraw`"
              class="btnfont btn btn-dark walbtn"
              style="font: 16px 'Yekan'"
              >برداشت</router-link
            >
      ...
      <tbody v-if="info3 && sorted" style="font: 20px 'UD'; max-height: 320px">
    ...
    <div
      ...
      style="min-height: 490px; padding: 50px; text-align: right"
    >
      <h2 style="text-align: center">چرا آمیزاکس ؟</h2>
      <br /><br />
      <div
        v-for="(item, idx) in bottom"
        v-bind:key="idx + 'e'"
        style="
          width: 25%;
          padding: 2%;
    ...
    >
      ...
      <div
        ...
        class="cent"
      >
        <img loading="lazy" style="width: 150px" :src="item.get_pic" alt="" />
        <h4 class="cent">{{ item.title }}</h4>
        <br />
        <p>{{ item.minitext }}</p>
      </div>
    ...
    <h2
      v-if="posts.length"
      style="height: 40px; margin: 0; padding: 10px; text-align: center"
    >
      <br />
      آخرین اخبار
    </h2>
    ...
    >
      <div
        style="
          ...
          width: 80%;
          margin: auto;
          border: solid rgba(125, 125, 125, 0.5) 0.5px;
          text-align: right;
        "
      >
    ...
    <footer
      class="d-none d-md-block"
      style="background: #222; color: white; text-align: right"
    >
      <div class="col-12" style="height: 250px">
        ...
        <div class="col-3" style="height: 100%; float: right; padding: 2%">
          ...
          <br />
          <router-link to="/pages/margin-trade"
            ><h6 style="color: grey; font-size: 14px">
              معاملات مرجین
            </h6></router-link
          >
          <router-link to="/pages/perpetual-trade"
            ><h6 style="color: grey; font-size: 14px">
              معاملات پرپشوال
            </h6></router-link
          >
          <router-link to="/pages/exchange"
            ><h6 style="color: grey; font-size: 14px">اکسچینج</h6></router-link
          >
          <router-link to="/pages/buysell"
            ><h6 style="color: grey; font-size: 14px">
              خرید و فروش
            </h6></router-link
          >
          <router-link to="/pages/onlinetrading"
            ><h6 style="color: grey; font-size: 14px">
              تریدینگ لحظه ای
            </h6></router-link
          >
        ...
        <div class="col-3" style="height: 100%; float: right; padding: 2%">
          ...
          <br />
          <router-link to="/pages/group"
            ><h6 style="color: grey; font-size: 14px">
              اعضای شرکت
            </h6></router-link
          >
          <router-link to="/pages/history"
            ><h6 style="color: grey; font-size: 14px">تاریخچه</h6></router-link
          >
          <router-link to="/pages/line"
            ><h6 style="color: grey; font-size: 14px">
              زمینه فعالیت
            </h6></router-link
          >
          <router-link to="/pages/rules"
            ><h6 style="color: grey; font-size: 14px">
              قوانین و مقررات
            </h6></router-link
          >
    ...
    <footer
      class="d-md-none"
      style="background: #222; color: white; text-align: center"
    >
      <div class="col-12" style="height: auto">
        <div class="col-12" style="height: 100%; padding: 2%">
          <b-navbar-brand to="/" style="margin: 0">
            <img
              loading="lazy"
              style="width: 60px; height: 70px; margin: auto"
              src="/img/brand/ars.png"
            /> </b-navbar-brand
          ><br /><br />
...
export default {
    ...
    sortedinfo: function () {
      const sortable = Object.fromEntries(
        Object.entries(this.info3).sort((a, b) => {
          return (
            parseFloat(b[1][this.currentSort]) -
            parseFloat(a[1][this.currentSort])
          );
        })
      );
      return sortable;
  ...
  methods: {
    ...
    async price() {
      ...
      await axios.get("/indexprice").then((response) => {
    ...
    async gettop() {
      await axios.get("/topsticker").then((response) => {
        var temp = [];
        var i = 0;
        for (var j = 3; j - 3 <= response.data.length; j = j + 3) {
          ...
          for (i; i < j; i++) {
            if (response.data[i]) {
              temp.push({
                get_pic: response.data[i].get_pic,
                title: response.data[i].title,
                text: response.data[i].text,
                minitext: response.data[i].minitext,
              });
            }
          }
          i = j;
          if (temp.length) {
            if (temp.length < 3) {
              for (var f = 0; f < 3 - temp.length; f++) {
                temp.push({
                  get_pic: response.data[f].get_pic,
                  title: response.data[f].title,
                  text: response.data[f].text,
                  minitext: response.data[f].minitext,
                });
              }
            }
    ...
    async getbottom() {
      await axios.get("/bottomsticker").then((response) => {
    ...
    async getposts() {
      await axios.get("/mainpageposts").then((response) => {
        this.posts = response.data;
      });
    },
    async history() {
      await axios.get("/indexhistory").then((response) => {
